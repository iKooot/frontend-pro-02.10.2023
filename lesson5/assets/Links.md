# Документації

## String

[String](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/String)

[Корисні методи строк](https://developer.mozilla.org/ru/docs/Learn/JavaScript/First_steps/Useful_string_methods)

[UTF-8](https://ru.wikipedia.org/wiki/UTF-8)

[UTF-16](https://ru.wikipedia.org/wiki/UTF-16)

[Специфікація](https://tc39.es/ecma262/multipage/text-processing.html#sec-string-objects)

## Number

[Number](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Number)

[Специфікація](https://tc39.es/ecma262/#sec-number-objects)

## BigInt

[BigInt](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/BigInt)

[Специфікація](https://tc39.es/ecma262/#sec-bigint-objects)

## Boolean

[Boolean](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Boolean)

[Специфікація](https://tc39.es/ecma262/#sec-boolean-objects)

## null

[null](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Operators/null)

[Специфікація](https://tc39.es/ecma262/multipage/overview.html#sec-null-value)

## undefined

[undefined](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/undefined)

[Специфікація](https://tc39.es/ecma262/multipage/global-object.html#sec-undefined)

## Symbol

[Symbol](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Symbol)

[Специфікація](https://tc39.es/ecma262/multipage/fundamental-objects.html#sec-symbol-objects)
